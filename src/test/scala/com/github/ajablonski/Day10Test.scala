...
class Day10Test extends AnyFlatSpec with Matchers with AocTestData(10) {
  ...
  "removeMatchedChunks" should "return a line with all matched chunks removed" in {
    Day10.removeMatchedChunks("{([(<{}[<>[]}>{[]{[(<()>") shouldBe "{([(<[}>{{[("
    Day10.removeMatchedChunks("[({(<(())[]>[[{[]{<()<>>") shouldBe "[({([[{{"
  }
  ...
  "complete" should "return the characters needed to complete the line" in {
    Day10.complete("[({(<(())[]>[[{[]{<()<>>") shouldBe "}}]])})]"
    Day10.complete("[(()[<>])]({[<{<<[]>>(") shouldBe ")}>]})"
    Day10.complete("(((({<>}<{<{<>}{[]{[]{}") shouldBe "}}>}>))))"
    Day10.complete("{<[[]]>}<{[{[{[]{()[[[]") shouldBe "]]}}]}]}>"
    Day10.complete("<{([{{}}[<[[[<>{}]]]>[]]") shouldBe "])}>"
  ...
  it should "multiply previous score by 5 before adding for multi-character completions" in {
    Day10.scoreCompletion("}}]])})]".split("").toList.asInstanceOf[List[ClosingChar]]) shouldBe 288957
    Day10.scoreCompletion(")}>]})".split("").toList.asInstanceOf[List[ClosingChar]]) shouldBe 5566
    Day10.scoreCompletion("}}>}>))))".split("").toList.asInstanceOf[List[ClosingChar]]) shouldBe 1480781
    Day10.scoreCompletion("]]}}]}]}>".split("").toList.asInstanceOf[List[ClosingChar]]) shouldBe 995444
    Day10.scoreCompletion("])}>".split("").toList.asInstanceOf[List[ClosingChar]]) shouldBe 294
  }
}
